...
describe("inspector and action copy", () => {
  ...
  it("labels top-level activation as startLearning and child activation as enterQuestion", () => {
    const { snapshot, ids } = createBlockedBranchFixture();
...
describe("commands", () => {
  ...
  it("switches sibling branches through activateNode and leaves the old child inactive", () => {
    ...
    expect(switched.snapshot.pass.activeStack).toEqual([ids.parent,
      ids.childB,
    ]);
  ...
  it("resumes a parked node onto the Active Stack without inventing focus", () => {
    ...
    expect(resumed.snapshot.pass.activeStack).toEqual([ids.q1,
      ids.q12,
    ]);
