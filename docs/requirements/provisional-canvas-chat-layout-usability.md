# Canvas & AI Panel Interaction Usability

provisional:
  slug: canvas-chat-layout-usability
...
Controls must describe what clicking will do, not merely repeat current state.
...
- Do not render mutually exclusive current-state labels at the same time in a way that makes the action ambiguous.
...
A learning question must render as one visual node container.
...
### 4. Add one-click “show all nodes” viewport action
...
1. Floating panel exposes `停靠`, not a second ambiguous current-state label.
